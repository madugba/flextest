...
import { buildSocketState, notifyStatusSubscribers } from './socket-client.helpers';
import type { StatusCallback, TypedSocket } from './socket-client.types';

export type { StatusCallback, TypedSocket } from './socket-client.types';
...
class SocketClient {
  ...
  private notifyStatusChange(status: ConnectionStatus, error?: Error): void {
    notifyStatusSubscribers(
      ...
      buildSocketState(status, this.reconnectAttempts, error)
    );
  }
