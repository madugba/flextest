import { apiClient, ApiError, type ApiResponse } from '@/shared/api/client'
import type {
  Question,
...
/**
 * Update question — routed through the Next.js server proxy to avoid
 * cross-origin network errors that can occur with browser-to-backend PATCH calls.
 */
export async function updateQuestion(
  ...
  const result = (await res.json()) as ApiResponse<Question>
...
export async function getQuestionsBySubjectAndSession(
  ...
  const response = await apiClient.get<Question[]>(
    `${QUESTION_ENDPOINTS.BY_SUBJECT_SESSION}?${params.toString()}`
  )
