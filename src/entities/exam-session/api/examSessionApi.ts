 ...
 * Served by the local Next.js API route which computes scores directly from
 * the database using candidate_answers, questions, and score_configurations.
 */
export async function getSessionScores(sessionId: string): Promise<SessionScores> {
  ...
  return response.json() as Promise<SessionScores>
}
